# -*- coding: utf-8 -*-
"""
Created on Thu May 25 15:43:54 2023
...
f = open("log-ped"+timestamp+".txt", "a")
...
for i in glob.glob('C:\your-directory\Scripts\example-informal\\**\\*.PBF',recursive=True):
    try:
        file = i
        ...
        #load in all roads - because the input file is pre-processed to only contain informal roads the code is the same as the Roads.py file - just the input files are different
        highway= osm.get_network(network_type='all',extra_attributes=['bridge:structure'])
...
with pd.ExcelWriter('Informal_roadnetwork'+timestamp+'.xlsx') as writer:  
